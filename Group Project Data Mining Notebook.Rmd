...
```{r - Regression on the imputation}

#First, turn teh datasets into long format
okcupid_long = mice::complete(imp2, action = "long", include = TRUE)

# Convert factor variables into numeric 
okcupid_long$body_type = as.numeric(okcupid_long$body_type)
okcupid_long$diet = as.numeric(okcupid_long$diet)
okcupid_long$drinks = as.numeric(okcupid_long$drinks)
okcupid_long$drugs = as.numeric(okcupid_long$drugs)
okcupid_long$education = as.numeric(okcupid_long$education)
okcupid_long$ethnicity = as.numeric(okcupid_long$ethnicity)
okcupid_long$job = as.numeric(okcupid_long$job)
okcupid_long$orientation = as.numeric(okcupid_long$orientation)
okcupid_long$pets = as.numeric(okcupid_long$pets)
okcupid_long$sex = as.numeric(okcupid_long$sex)
okcupid_long$smokes = as.numeric(okcupid_long$smokes)
okcupid_long$status = as.numeric(okcupid_long$status)

# Convert to mids type - mice can work with this type
okcupid_long_mids = as.mids(okcupid_long)

# Regression

fitimp = with(okcupid_long_mids, lm(height ~ age + body_type + diet + drinks + drugs + education)); fitimp
pool(fitimp)
summary(pool(fitimp))
...
kNN_pred_Nora = class::knn(train = okcupid.norm[, c(1, 3:4, 6:7, 9:13)], test = Nora_norm, cl = okcupid.norm[, 15], k = 2); kNN_pred_Nora
...
kNN_pred_Greg = class::knn(train = okcupid.norm[, c(1, 3:4, 6:7, 9:13)], test = Greg_norm, cl = okcupid.norm[, 15], k = 2); kNN_pred_Greg
...
kNN_pred_Augusta = class::knn(train = okcupid.norm[, c(1, 3:4, 6:7, 9:13)], test = Augusta_norm, cl = okcupid.norm[, 15], k = 2); kNN_pred_Augusta
